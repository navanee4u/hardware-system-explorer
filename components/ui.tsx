...
export function Scorecard({ sc }: { sc: ScoreCard }) {
  return (
    <div style={{ display: "grid", gap: 7 }}>
      <div style={{ fontSize: 11.5, color: "var(--rf-muted)", lineHeight: 1.35 }} title="SWAP-C sub-scores — higher = better (0–1)">
        SWAP-C sub-scores · higher = better
      </div>
      {SCORE_ROWS.map((r) => (
        <div key={r.key} style={{ display: "grid", gridTemplateColumns: "minmax(0, 80px) 1fr 30px", gap: 6, alignItems: "center" }} title={r.hint}>
          <span style={{ fontSize: 11.5, color: "var(--rf-muted)", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.label}</span>
          <Bar value={sc[r.key]} />
          <span style={{ fontSize: 11.5, fontFamily: "var(--rf-mono)", textAlign: "right" }}>{sc[r.key].toFixed(2)}</span>
        </div>
      ))}
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 80px) 1fr 30px", gap: 6, alignItems: "center", borderTop: "1px solid var(--rf-border)", paddingTop: 7 }} title="Weighted blend of the sub-scores above — drives the ranking">
        <span style={{ fontSize: 12, fontWeight: 700 }}>Overall</span>
        <Bar value={sc.composite} />
        <span style={{ fontSize: 12, fontFamily: "var(--rf-mono)", textAlign: "right", fontWeight: 700 }}>{sc.composite.toFixed(2)}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 11.5, fontFamily: "var(--rf-mono)", color: coverage >= 1 ? "var(--rf-pass)" : "var(--rf-muted)" }} title="Hard requirements that must all pass for a design to be valid">
          {passed}/{hard.length} hard · {Math.round(coverage * 100)}%
        </span>
      </div>
      <div style={{ fontSize: 11.5, color: "var(--rf-muted)", lineHeight: 1.35, marginBottom: 8 }}>
        Every hard requirement must pass; soft goals are nice-to-have.
      </div>
      {checks.map((c) => (
        <div className="check" key={c.constraint_id} title={c.reason} style={{ fontSize: 12, lineHeight: 1.4 }}>
          <span className={`dot dot-${c.status === "pass" ? "pass" : "fail"}`} />
          <span style={{ color: c.kind === "soft" ? "var(--rf-muted)" : "inherit", minWidth: 0, overflowWrap: "anywhere" }}>
            {c.constraint_id}
            {c.kind === "soft" ? " (soft)" : ""}
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div>
        ...
        <div style={{ fontSize: 11.5, color: "var(--rf-muted)", lineHeight: 1.35, marginTop: 2 }}>
          Parts grouped by subsystem. Source badges:
          {" "}<span style={{ fontFamily: "var(--rf-mono)" }}>kb</span> = knowledge base,
      ...
      {entries.map(([subsystem, list]) => (
        <div key={subsystem}>
          ...
          {(list as Component[]).map((c) => (
            <div key={c.id} style={{ display: "flex", gap: 6, alignItems: "baseline", flexWrap: "wrap", fontSize: 12.5, lineHeight: 1.4 }}>
              <Provenance source={c.source} />
              <span style={{ minWidth: 0, overflowWrap: "anywhere" }}>{c.name}</span>
              <span style={{ color: "var(--rf-muted)", fontFamily: "var(--rf-mono)", fontSize: 11.5, overflowWrap: "anywhere" }}>{c.part_number}</span>
            </div>
          ))}
        </div>
      ))}
