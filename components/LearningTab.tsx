...
export function LearningTab() {
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div className="card">
        <span className="eyebrow">The self-learning loop</span>
        <p style={{ margin: "8px 0 0", fontSize: 14, lineHeight: 1.5, maxWidth: 760 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap", fontSize: 11, color: "var(--rf-muted)", fontFamily: "var(--rf-mono)" }}>
      ...
      <div className="card">
        ...
        <div style={{ fontSize: 12, color: "var(--rf-muted)", margin: "4px 0 12px" }}>
          How human choices have reshaped the SWAP-C composite (each axis starts at 0.20).
        </div>
          ...
          {AXES.map((axis) => {
            ...
            return (
              <div key={axis} style={{ display: "grid", gridTemplateColumns: "150px 1fr 120px", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 13 }}>{AXIS_LABEL[axis]}</span>
        ...
        {decisions.length === 0 ? (
          ...
          <div style={{ overflowX: "auto", marginTop: 10 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
              <thead>
                <tr style={{ textAlign: "left", color: "var(--rf-muted)", fontSize: 11 }}>
      ...
      {shownExamples.length > 0 && (
        <div className="card">
          ...
          <div style={{ fontSize: 12, color: "var(--rf-muted)", margin: "4px 0 14px" }}>
            The agent&apos;s #1 vs the design the human actually chose. The axes where the human&apos;s pick
            scored higher are exactly what the loop learned to weight more.
          </div>
                  ...
                  <div style={{ fontSize: 12.5, marginBottom: 10 }}>
                    ...
                    {d.agreed ? (
                      ...
                      <span style={{ color: "var(--rf-secondary)", marginLeft: 6 }}>
                        ↔ chose {d.chosen} over #1 {d.agentTop}
                        {favored.length ? ` → favored ${favored.map((a) => AXIS_LABEL[a].toLowerCase()).join(", ")}` : ""}
                      </span>
                    )}
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <ExampleDesign label={`Agent #1 · ${d.agentTop}`} profile={d.agentTop} sc={top} dim={!d.agreed} />
                    {d.chosen && <ExampleDesign label={`Human chose · ${d.chosen}`} profile={d.chosen} sc={pick} highlight={!d.agreed} />}
                  </div>
                </div>
              );
            })}
...
function StatTile({ label, value, sub, trend }: { label: string; value: string; sub?: string; trend?: { ts: string; rate: number }[] }) {
  return (
    <div className="card" style={{ padding: 16 }}>
      <div style={{ fontSize: 11, color: "var(--rf-muted)" }}>{label}</div>
      ...
      {sub && <div style={{ fontSize: 10.5, color: "var(--rf-muted)", marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
...
function ExampleDesign({ label, profile, sc, dim, highlight }: { label: string; profile: Profile; sc?: ScoreCard; dim?: boolean; highlight?: boolean }) {
  return (
    <div
      style={{
    ...
    >
      <div style={{ fontSize: 12, fontWeight: 600, color: CANDIDATE_COLOR[profile], marginBottom: 8 }}>{label}</div>
      {sc ? <Scorecard sc={sc} /> : <div style={{ fontSize: 12, color: "var(--rf-muted)" }}>scorecard unavailable</div>}
    </div>
  );
}

function Profile({ p }: { p: Profile }) {
  return <span style={{ color: CANDIDATE_COLOR[p], fontWeight: 600 }}>{p}</span>;
}
